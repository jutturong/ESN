...
public	function test()
 ...
 public function  load_appendix()//ใช้สำหรับการ load appendix 1-6
	...
	public   function  checklogin() // ตรวจสอบการเข้าใช้ระบบ
	{
	...
	public function  home_loading()  //หน้าหลักโปรแกรม
	{
	       $sess_num = $this->session->userdata('sess_num'); 
	       if( $sess_num == 1 )
		   {
	...
	public function   province() //เลือกจังหวัด
	{
	      $this->db->select('*');
	...
	public function  load_province()//เืลือกจังหวัด
	{
	...
	public    function  load_district() //โหลดตำบลจาก jquery
		{
		     $this->db->select('*');
		...
		public function  load_select() //id,tb,id_field,name_field ใช้สำหรับเลือก จังหวัด  ตำบล  อำเภอ
			{
	...
	public function   select_province() //เลือกจังหวัด  =================> modul ตัวใหม่ ที่ทดสอบจากการแยก
	{
   ...
   public function  select_amphur2() //เลือกอำเภอจาก jquery
   {
 ...
 public  function  select_amphur3()
   {
   ...
   public function  select_district3()
   {
	...
	public function  select_district2() //เลือกตำบล จาก jquery
	{
	...
	public function select_district() //ตำบล =================> modul ตัวใหม่ ที่ทดสอบจากการแยก
	{
			$district_id=trim($this->input->post('district_id'));
			if (  $district_id  >  0 )
			{	  
...
//			            echo  '{"total":"'.$total.'","result":'.$jsonresult.'}'; 
	...
	public function  loaddojo() //ทดสอบการโหลด dojo
	{
	      $data['title']= title_program();
		  $this->load->view('',$data);
	...
	public function   sql_query() //เขียนการใช้ sql  เพื่อทดสอบการเลือกจังหวัด
    {
	     $tb="main_amphur";
		 $id="amphur_id";
		 $value=81;
...
//		 $this->db_query->query_amphur($tb,$id,$value,$name);  //ทดสอบการดึงชื่ออำเภอ
     ...
     public function   SelectAmphur() //เลือกอำเภอ
	 {
...
//			  echo  br(); 
       ...
       public function  search_() //การค้นหาข้อมูล
	   {
	         $sess_num = $this->session->userdata('sess_num'); 
				   if( $sess_num == 1 )
				   {
		...
		public function  test_combo() //ทดสอบการเขียน combobox
		{
		     $data['title']= title_program();
		...
		public function   load_add_user() //load  add user
		{
				   $sess_num = $this->session->userdata('sess_num'); 
				   if( $sess_num == 1 )
				   {
		...
		public function  load_home_admin() //หน้า load สำหรับ การเพิ่ม user
		{
...
*/		     	
		...
		public function  topMenu() //ทดสอบ TopMenu
		{
		            $data['title']= title_program();
					$this->load->view('topMenu/topMenu',$data);
		}
		public function  load_begin_page() //โหลดหน้าแรกเมื่อเข้าสู่ระบบ login
		{
		   $sess_num = $this->session->userdata('sess_num'); 
		   $data['test']="test";
				   if( $sess_num == 1 )
	...
	public function  province_autocomplete() //ค้นหาชื่อจังหวัีดแบบ auto complete
	{
...
//						 }	
	...
	public function  search_all() //ค้นหา ทั้งหมดจาก ช่องเดียว HN ชื่อ นามสกุล
	{
	     		     $sess_num = $this->session->userdata('sess_num'); 
			 	 if( $sess_num == 1 )
				   {
	 ...
	 public function  all_autocomplete_HN()//ค้นหาผู้ป่วยจาก HN จากการกดปุ่ม
		{
		     $sess_num = $this->session->userdata('sess_num'); 
			 	 if( $sess_num == 1 )
				   {
		...
		public function  autocomplete_HN()//ค้นหาผู้ป่วยจาก HN
		{
		     $sess_num = $this->session->userdata('sess_num'); 
			 	 if( $sess_num == 1 )
				   {
		...
		public function  app_autocomplete_HN()//แสดง HN จาก การค้นหา appendix
		{
		     $sess_num = $this->session->userdata('sess_num'); 
			 	 if( $sess_num == 1 )
				   {
		...
		public function  autocomplete_name()//ค้นหาผู้ป่วยจาก ชื่อ
		{
		     $sess_num = $this->session->userdata('sess_num'); 
			 	 if( $sess_num == 1 )
				   {
		...
		public function  autocomplete_lastname()//ค้นหาผู้ป่วยจาก นามสกุล
		{
		     $sess_num = $this->session->userdata('sess_num'); 
			 	 if( $sess_num == 1 )
				   {
		...
		public function  load_progress_note()//โหลด progress note สำหรับ appendix 2 ใน  2.2
		{
