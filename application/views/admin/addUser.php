...
Ext.extend(Ext.form.XmlErrorReader, Ext.data.XmlReader);
	...
	var   id_typeuser_edit=new  Ext.form.ComboBox({  //ประเภทของผู้ใ้ช้
		      triggerAction:'all',
		  lazyRender:true,
	      fieldLabel:'แสดงประเภทของ user ',
		  id:'id_typeuser_edit',
		  name:'id_typeuser_edit', 
   ...
   var   valueProvince_edit=new  Ext.data.JsonStore({
				 	      url:  '<?=base_url()?>index.php/home/select_province',
						 root: 'result',
  					     fields: ['prov_id','prov_name']
						                         });
//===============================เืลือกจังหวัด form update user========================================================	
	 var  edit_id_province =new Ext.form.ComboBox({  //เลือกจังหวัด
	      triggerAction:'all',
		  lazyRender:true,
	      fieldLabel:'จังหวัด',
		  id:'edit_id_province',
		  name:'edit_id_province', 
	      emptyText:'เลือกจังหวัด',
		  displayField:'prov_name',
		  valueField:'prov_id',
		  store:valueProvince_edit, 
     	  typeAhead:true,
		   mode:'remote',
		   forceSelection:true,
...
var   userFormJson=new  Ext.FormPanel({
		 ...
		 reader : new Ext.data.JsonReader({
		          root:'results',
		          fields:[
				        { name:'id_user',mapping:'id_user' }
						,
						{ name:'edit_user',mapping:'user'  }
						,
						//{ name:'edit_password',mapping:'password' }
						{ name:'edit_name',mapping:'name' }
						,
						//ปรับปรุง จังหวัด
						 ,
						 {  name:'edit_id_province',mapping:'id_province',convert:function(v,rec){
						 
						                                                                          } }
						 ,
						{ name:'edit_code_user',mapping:'code_user' }
					    ,
						{ name:'edit_code_hospital',mapping:'code_hospital' }
						//ปรับปรุง สิทธิการใช้
						
						
						,
						{ name:'id_typeuser_edit',mapping:'id_typeuser',convert:function(v,rec)
						        {
								  
								      if(v==1)
									  {
									     //alert('1');
										 //type_edit.options.options[1].text;
										 //type_edit.options[1].text="";
										 //v="Admin";
										 document.getElementById('show_typeuser').value="Admin";
										
									  }
									  else if( v==2 )
									  {
									    //alert('2');
										 document.getElementById('show_typeuser').value="User";
									  }
									  //return v;
								 } }
				  ]
								   ...
								   [
								       {
														  ...
														  handler:function()
														  {
															  if( userFormJson.getForm().isValid() )
															  {
																  userFormJson.getForm().submit({
																	 url:'<?=base_url()?>index.php/admin/update_user',
																	 method:'POST',
																	 success:function(form,action)
																		 {
																				PresidentsDataStore.load();
																				var   dialog_update=Ext.MessageBox.show({ //แสดง dialog หลังจาก update
																															 title:'Update Status',
																															 msg:'ปรับปรุงข้อมูลแล้ว',
																															 width:'300',
